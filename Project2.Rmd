...
#### Final Model
We compare the final models selected from `tree.cv` and `boost.cv` for best performance on the test dataset. We measure performance as the smallest RMSE (root mean squared error), which reflects unexplained variation. Then, we'll take the best model and report it's parameters.  
...
```
We prefer the `r bestMethod` because it has lower RMSE.
